...
export default async function setUserProfil(req, res) {
    ...
    try {
        ...
        if (user.level === "expert") {
            console.log("expert")
            const {name, lastName, birtdate, gender, address, points, level, isActive} = req.body.data;
            if (!name || !lastName || birtdate || !gender || !address || !points || !level || !isActive) {
        ...
        // 2. it's your account
            ...
            const {name, lastName, birtdate, gender, address} = req.body.data;
            if (!name || !lastName || birtdate || !gender || !address) {
                return res.status(401).json({error: 'Invalid data: missing required fields.'});
            }
